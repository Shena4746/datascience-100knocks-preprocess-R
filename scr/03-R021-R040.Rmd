...
```{r}
customer_top20_amount <- customer_tbl %>%
...
receipt_tbl %>%
    ...
    group_by(customer_id) %>%
    summarise(n_days = n_distinct(sales_ymd)) %>%
    arrange(desc(n_days)) %>%
    head(20) %>%
    full_join(customer_top20_amount %>% head(20), by = "customer_id") %>%
    head()
```
