...
```{r R083-2}
# delete tmp table if there is the one
...
product_tbl %>%
    ...
    group_modify(
        ~ mutate(
            ...
            across(
                .cols = all_of(rows_to_impute),
                .fns = ~ coalesce(.x, cur_data() %>% summarise(across(stringr::str_c(cur_column(), "_med"), first)) %>% pull(1))
            )
        )
    ) %>%
    select(-ends_with("_med")) %>%
    # create a temporary table in server
...
tbl(con, "tmp") %>%
    ...
    # check that the merged table has no NA cells
